...
def _get_test_data_loader(test_batch_size, training_dir):
    dataset = pd.read_csv(os.path.join(training_dir, "validation.csv"))
    sentences = dataset.text.values
    labels = dataset.label.values
...
def train(args):
    ...
    model = BertForSequenceClassification.from_pretrained(
        ...
        num_labels=args.num_labels,  # The number of output labels--3
...
def input_fn(request_body, request_content_type):
    ...
    if request_content_type == "application/json":
        ...
        #encoded = [tokenizer.encode(x, add_special_tokens=True, max_length=MAX_LEN, truncation=True) for x in data]
        #encoded = tokenizer(data, add_special_tokens=True, max_length=MAX_LEN, truncation=True) 
        
        # for backward compatibility use the following way to encode 
        # https://github.com/huggingface/transformers/issues/5580
        input_ids = [tokenizer.encode(x, add_special_tokens=True, max_length=MAX_LEN, truncation=True) for x in data]
